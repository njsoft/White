...
namespace White.Core.UnitTests.UIItems.Finders
{
    ...
    public class ControlTypeConditionTest
    {
        ...
        {
            Assert.AreEqual("ControlType=button", SearchConditionFactory.CreateForControlType(typeof(Button), null).ToString());
            Assert.AreEqual("ControlType=pane", SearchConditionFactory.CreateForControlType(typeof(TestCustomUIItem), null).ToString());
